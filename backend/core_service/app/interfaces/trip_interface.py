...
#---------------------------------GET Functions----------------------------------------
    ...
    def get_trips_by_city_name(self, city_name: str, offset: int, limit: int):
        total_count_query = (
            self.db.query(func.count(Trip.trip_id))
            .filter(
                Trip.is_deleted == False,
                Trip.status == True,
                Trip.trip_destination == city_name
            )
            .scalar_subquery()
        )
    ...
    def create_trip_search(self, trip_search_request: CreateTripSearchRequest, offset: int, limit: int):
        try:
            ...
            search_log_filters = {
                ...
                "search_parameters": {
                    "trip_search_origin": trip_search_request.trip_search_origin,
                    "trip_search_destination": trip_search_request.trip_search_destination,
                    "trip_search_start_date": trip_search_start_date,
                    "trip_search_end_date": trip_search_end_date
                }
            }
